...
* C++ source files:
  * `include/example-cookiecutter-cpp-project/example-cookiecutter-cpp-project.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/example-cookiecutter-cpp-project.cpp` is the main file that implements this library.
  * `app/example-cookiecutter-cpp-project_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/example-cookiecutter-cpp-project_t.cpp` contains the unit tests for the library.
  ...
  * The `python/examplecookiecuttercppproject` directory contains a Python
    package for the project. It contains a compiled Python module `_examplecookiecuttercppproject`
    that CMake generates from the Pybind11 source file `python/examplecookiecuttercppproject/_example-cookiecutter-cpp-project.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
...
* CMake build system files
  ...
  * `example-cookiecutter-cpp-projectConfig.cmake.in` provides a template for the configuration
    installed alongside your project. This is required to implement the transitivity of your dependency
    on `FAKE_DEPENDENCY`: If downstream projects use your library, they should
    automatically search for `FAKE_DEPENDENCY`. The config file template implements
    exactly this logic.
