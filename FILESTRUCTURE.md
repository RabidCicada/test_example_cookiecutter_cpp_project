...
* C++ source files:
  * `include/test-cookiecutter-cpp-project/test-cookiecutter-cpp-project.hpp` is the main
    C++ header that declares the interface of your library.
  * `app/test-cookiecutter-cpp-project_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/test-cookiecutter-cpp-project_t.cpp` contains the unit tests for the library.
  ...
  * The `python/testcookiecuttercppproject` directory contains a Python
    package for the project. It contains a compiled Python module `_testcookiecuttercppproject`
    that CMake generates from the Pybind11 source file `python/testcookiecuttercppproject/_test-cookiecutter-cpp-project.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
